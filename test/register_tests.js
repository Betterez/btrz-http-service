...
describe("attachHandlerToExpressServer()", () => {
  ...
  let handlerConfiguration = null;
  let authenticator = null;
  let btrzSwaggerExpress = null;
  ...
  describe("when registering a handler class which does not have a register() function", () => {
    it("should call the handler's constructor with the correct arguments", () => {
      ...
      class HandlerClass {
        ...
        getSpec = mock.fn(() => { return handlerSpec; });

        configuration = mock.fn(() => { return handlerConfiguration; });
    ...
    it("should allow the handler class' getSpec() method to be static", () => {
      class HandlerClass {
        static getSpec = mock.fn(() => { return handlerSpec; });
    ...
    it("should throw an error if the handler class has no getSpec() method", () => {
      ...
      assert.throws(
        () => { return attachHandlerToExpressServer(HandlerClass, models, dependencies); },
        (error) => { return error.message === "HandlerClass has no OpenAPI specification.  The handler class must contain a 'getSpec()' function, which returns the OpenAPI specification for the endpoint."; }
      );
    });
    ...
    it("should throw an error if the handler class has no configuration() method", () => {
      ...
      assert.throws(
        () => { return attachHandlerToExpressServer(HandlerClass, models, dependencies); },
        (error) => { return error.message === "HandlerClass has no configuration.  The handler class must contain a 'configuration()' function, which returns a configuration object."; }
      );
    });
    ...
    it("should throw an error if the handler configuration has no authorization policy", () => {
      ...
      assert.throws(
        () => { return attachHandlerToExpressServer(HandlerClass, models, dependencies); },
        (error) => { return error.message === "HandlerClass has no authorization policy.  The 'configuration()' function must return an object with an 'authorization' property which describes how requests should be authorized."; }
      );
    });
    ...
    it("should throw an error if the handler instance has no 'handler()' method", () => {
      ...
      assert.throws(
        () => { return attachHandlerToExpressServer(HandlerClass, models, dependencies); },
        (error) => { return error.message === "HandlerClass has no 'handler(req, res)' method.  The class must contain a handler function which will receive incoming requests."; }
      );
    });
    ...
    it("should attach the handler to the Express server, allowing it to respond to requests", async () => {
      class HandlerClass {
        ...
        handler = mock.fn(() => { return "Some endpoint response"; });
      }
    ...
    it("should return WRONG_DATA when required body is expected but req.body is undefined", async () => {
      class HandlerClass {
        getSpec = mock.fn(() => { return handlerSpec; });
    ...
    it("should throw an error if the 'validationSettings' in the handler configuration is not an object", () => {
      ...
      assert.throws(
        () => { return attachHandlerToExpressServer(HandlerClass, models, dependencies); },
        (error) => { return error.message === "HandlerClass has invalid 'validationSettings'.  The 'validationSettings' returned by the 'configuration()' function should be an object."; }
      );
    });
    ...
    it("should allow custom middleware to be provided in the handler configuration, and execute that middleware after the authorization middleware but before the handler function", async () => {
      ...
      mock.method(authenticator, "getMiddlewareForAuthPolicy", () => {
    ...
    it("should return the expected response if the custom middleware yields an error", async () => {
      handlerConfiguration.middleware = [
        mock.fn((req, res, next) => { return next(new Error("Some error from middleware")); })
      ];
    ...
    it("should throw an error if the 'middleware' in the handler configuration is not an array", () => {
      ...
      assert.throws(
        () => { return attachHandlerToExpressServer(HandlerClass, models, dependencies); },
        (error) => { return error.message === "HandlerClass has invalid 'middleware'.  The 'middleware' returned by the 'configuration()' function should be an array."; }
      );
    });
