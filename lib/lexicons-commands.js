...
async function saveOrUpdate(apiClient, xApiKey, jwtToken, lexiconEntries) {
  try {
    const results = await apiClient.accounts.lexicons.createOrUpdate({
      token: xApiKey,
      jwtToken,
      updates: lexiconEntries
    });
    return results.data;
  } catch (e) {
