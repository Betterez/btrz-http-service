...
function getRequiredBodyParameter(openApiSpec) {
  const parameters = Array.isArray(openApiSpec?.parameters) ? openApiSpec.parameters : [];
  return parameters.find((parameter) => {
    return parameter?.in === "body" && parameter.required === true;
  });
}

function validateIncomingRequestAgainstOpenApiSpec(handlerInstance, handlerConfiguration, openApiSpec, models, logger, req) {
  const defaultValidationSettings = {
    ...
    removeNullValuesFromObjects: false,
    removeNullValuesFromArrays: false,
  ...
  const requiredBodyParameter = getRequiredBodyParameter(openApiSpec);
  if (requiredBodyParameter && (req?.body === undefined || req?.body === null)) {
    const parameterName = requiredBodyParameter.name || "request body";
    throw new ValidationError("WRONG_DATA", `${parameterName} is required`);
  }
  ...
  const validationLogs = validationResult.logs.toLiteral();
  if (validationLogs.deletedProperties.length > 0) {
    logger.debug(`The incoming request contains data that is not described in the handler's schema.  ` +
      `The following properties were removed from the request body: ${validationResult.logs.formatDeletedProperties()}`
    );
  }
  if (validationLogs.nullValuesRemovedFromObjects.length > 0 || validationLogs.nullValuesRemovedFromArrays.length > 0) {
    logger.debug(`The incoming request contains 'null' values which have been removed.  ` +
      `The following values were removed from the request body: ` +
      [validationResult.logs.formatNullValuesRemovedFromObjects(), validationResult.logs.formatNullValuesRemovedFromArrays()]
...
function wrapHandlerWithStandardResponseProcessing(openApiSpec, models, logger, handlerConfiguration, handlerInstance) {
  return {
    ...
    async handler(req, res) {
      try {
        validateIncomingRequestAgainstOpenApiSpec(handlerInstance, handlerConfiguration, openApiSpec, models, logger, req);
